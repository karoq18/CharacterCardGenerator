...
function Bio() {
  ...
  const addTag = (tag, type = 'square') => {  
    ...
    const newText = `${textarea.value.substring(0, start)}${openingTag}${textarea.value.substring(start, end)}${closingTag}${textarea.value.substring(end)}`;

    updateFormData('biography', newText);
    updateFormData('previewBio', parseTagsToHTML(newText));
  };
  ...
  const handleContentChange = (e) => {
    ...
    updateFormData('biography', newValue);
    updateFormData('previewBio', parseTagsToHTML(newValue));
  };

  return (
    <div id="biographyMain">
      <div className={`con-tooltip single`}>
      <div className='h2'>Biografia</div>
        <Hint description={hintBio} /></div>
      <div className="editor-buttons">
        <button onClick={() => addTag('p', 'angle')}><img src={pIcon} alt="Nowy akapit" /></button>
        <button onClick={() => addTag('left', 'square')}><img src={leftIcon} alt="Left align" /></button>
        <button onClick={() => addTag('center', 'square')}><img src={centerIcon} alt="Center align" /></button>
        <button onClick={() => addTag('right', 'square')}><img src={rightIcon} alt="Right align" /></button>
        <button onClick={() => addTag('justify', 'square')}><img src={justifyIcon} alt="Justified" /></button>
        <button onClick={() => addTag('b', 'square')}><img src={boldIcon} alt="Pogrub" /></button>
        <button onClick={() => addTag('i', 'square')}><img src={italicIcon} alt="Kursywa" /></button>
        <button onClick={() => addTag('u', 'square')}><img src={underlinedIcon} alt="Podkreśl" /></button>
        <button onClick={() => addTag('strike', 'square')}><img src={strikeIcon} alt="Przekreśl" /></button>
      </div>
      <textarea
        className="biography"
        id="biography"
